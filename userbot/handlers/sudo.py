...
def init(client, sync_callback):
    ...
    async def rmsudo(event):
        ...
        if event.is_reply:
            ...
            # CHANGED: wrapped get_entity in try except
            # so removal still works even if entity fetch fails
            try:
                user = await client.get_entity(uid)
            except Exception:
                user = None
            ...
            # CHANGED: direct numeric ID support
          # without depending on Telethon entity resolution
            if arg.isdigit():
                uid = int(arg)

                # CHANGED: optional entity fetch only for display
                try:
                    user = await client.get_entity(uid)
                except Exception:
                    user = None
            ...
            await event.edit(
                "<b>Reply to a user or provide @username / user ID.</b>",
                parse_mode="html"
            )
            return
        ...
        # CHANGED:
        # if entity exists -> show clickable name
        # else -> show raw user ID
        if user:
            name = escape(get_display_name(user) or "Unknown")
            click_name = f'<a href="tg://user?id={uid}">{name}</a>'
        else:
            click_name = f"<code>{uid}</code>"
